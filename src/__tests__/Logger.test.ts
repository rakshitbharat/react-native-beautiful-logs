...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    (Logger as { instance?: Logger }).instance = undefined;
  });
  ...
  describe('file operations', () => {
    ...
    it(
      'should use correct file path based on platform',
      async () => {
        // Test Android path
    ...
    it(
      'should handle base64 encoding fallback',
      async () => {
        logger = Logger.getInstance();
        await new Promise(resolve => setTimeout(resolve, 100)); // Wait for initialization
        ...
        mockFs.appendFile
          .mockRejectedValueOnce(() => Promise.reject(utf8Error))
          .mockImplementationOnce((path: string, data: string, encoding: string) => {
    ...
    it(
      'should handle log rotation with file deletion errors',
      async () => {
        logger = Logger.getInstance();
        await new Promise(resolve => setTimeout(resolve, 100)); // Wait for initialization
        ...
        await logger.log('info', 'Test message');
        expect(console.error).toHaveBeenCalledWith('Error deleting log file:', deleteError);
      },
      TIMEOUT,
    );
    ...
    it(
      'should handle cleanup of non-existent files',
      async () => {
        logger = Logger.getInstance();
        await new Promise(resolve => setTimeout(resolve, 100)); // Wait for initialization
        mockFs.exists.mockResolvedValue(false);
    ...
    it(
      'should retry file operations on transient errors',
      async () => {
        logger = Logger.getInstance();
        await new Promise(resolve => setTimeout(resolve, 100)); // Wait for initialization
    ...
    it(
      'should handle log file cleanup on rotation',
      async () => {
        logger = Logger.getInstance();
        await new Promise(resolve => setTimeout(resolve, 100)); // Wait for initialization
    ...
    it('should handle concurrent log operations', async () => {
      const promises = Array(5)
        .fill(0)
        .map((_, i) => logger.log('info', `Message ${i}`));
    ...
    it('should handle file deletion errors', async () => {
      ...
      await logger.log('info', 'Test message');
      expect(console.error).toHaveBeenCalledWith('Error deleting log file:', expect.any(Error));
    });
  });
  ...
  describe('loggerInterface', () => {
    ...
    it('should cleanup old logs', async () => {
      mockFs.ls.mockResolvedValueOnce(['session_2024-03-01.txt', 'session_2024-03-14.txt']);
    ...
    it('should handle list files error', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Error listing log files:', expect.any(Error));
    });
    ...
    it('should handle cleanup errors', async () => {
      ...
      await logger.loggerInterface.deleteAllLogs();
      expect(console.error).toHaveBeenCalledWith('Error deleting log files:', expect.any(Error));
    });
  });
});
