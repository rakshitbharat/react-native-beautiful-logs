...
describe('Logger Demo', () => {
  ...
  it('demonstrates various log types and formatting', async () => {
    ...
    await info('[PerformanceMonitor]', 'Performance stats:', stats, 'Network status:', 'Connected');
...
function greet(name) {
  return \`Hello, \${name}!\`;
    ...
    // Error handling
    await error('[ErrorBoundary]', 'Failed to render component', {
    ...
    await warn('[Auth]', 'Multiple failed login attempts detected', {
    ...
    await info('[Fetch]', 'GET https://api.example.com/data', 'Response:', {
      status: 200,
